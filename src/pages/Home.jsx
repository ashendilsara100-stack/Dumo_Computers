...
export default function Home({ setPage, cart, setCart }) {
  ...
  useEffect(() => {
    ...
    planets = [new Planet(60, "#f59e0b", 0.2, 4), new Planet(100, "#78350f", 0.1, 2)];
  ...
  return (
    <div className="min-h-screen bg-black text-white relative overflow-hidden select-none">
      ...
      <div className="relative z-10">
        ...
        <section className="min-h-[90vh] flex items-center pt-20 px-6">
          <div className="max-w-7xl mx-auto w-full grid lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
              <span className="bg-amber-500/10 border border-amber-500 text-amber-500 px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-tighter mb-6 inline-block">
                PREMIUM GAMING GEAR
              </span>
              ...
              <div className="flex flex-col sm:flex-row gap-4 mt-8">
                <button onClick={() => setPage("shop")} className="px-8 py-4 bg-white text-black font-black rounded-xl hover:bg-amber-500 transition-all uppercase italic flex items-center justify-center gap-2">
                  SHOP NOW <ArrowRight size={20}/>
                </button>
                <button onClick={() => setPage("builder")} className="px-8 py-4 border-2 border-white font-black rounded-xl hover:bg-white/10 transition-all uppercase italic">
                  BUILD PC
                </button>
              </div>
            </motion.div>
            <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="hidden lg:flex justify-center relative">
              <div className="absolute inset-0 bg-amber-500/20 blur-[100px] rounded-full animate-pulse"></div>
              <img src="https://i.ibb.co/XrC6Y9fy/download-10-removebg-preview.png" className="w-full max-w-[500px] relative z-10 drop-shadow-[0_0_50px_rgba(245,158,11,0.3)]" alt="hardware"/>
            </motion.div>
          </div>
        </section>

        {/* TRUST BADGES */}
        <div className="bg-white text-black py-10 px-6 font-black uppercase italic overflow-hidden">
          <div className="max-w-7xl mx-auto flex flex-wrap justify-center md:justify-around gap-8 text-sm md:text-base">
            <div className="flex items-center gap-2"><Truck size={20}/> ISLANDWIDE DELIVERY</div>
            <div className="flex items-center gap-2"><ShieldCheck size={20}/> GENUINE WARRANTY</div>
            <div className="flex items-center gap-2"><Zap size={20}/> TECH SUPPORT</div>
          </div>
        </div>

        {/* FEATURED PRODUCTS */}
        <section className="max-w-7xl mx-auto px-6 py-24">
          <h2 className="text-3xl md:text-4xl font-black mb-12 italic uppercase border-l-8 border-amber-500 pl-4 tracking-tighter">FEATURED HARDWARE</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
            {products.map((p, i) => (
              <TiltCard key={p.id} index={i} className="bg-zinc-900/40 border border-white/5 p-5 md:p-6 rounded-[32px] backdrop-blur-xl hover:border-amber-500/50 transition-all group shadow-2xl">
        ...
        {/* CALL TO ACTION (FIXED FROM IMAGE) */}
        <section className="px-6 py-20">
          <motion.div 
            initial={{ opacity: 0, y: 50 }} 
            whileInView={{ opacity: 1, y: 0 }} 
            viewport={{ once: true }}
            className="max-w-5xl mx-auto bg-amber-500 py-16 md:py-24 px-8 rounded-[40px] md:rounded-[60px] text-black text-center relative overflow-hidden"
          >
            ...
            {/* Background Decor */}
            <div className="absolute top-0 right-0 w-32 h-32 bg-white/20 blur-[60px] rounded-full -mr-10 -mt-10"></div>
          </motion.div>
        </section>
      </div>
