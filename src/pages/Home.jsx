  ...
  <div className="w-full grid lg:grid-cols-2 gap-8 items-center max-w-[1600px] mx-auto">
    ...
    <div className="hidden lg:flex justify-center items-center relative animate-reveal-right" style={{ animationDelay: '0.3s' }}>
      ...
      />
        ...
        {/* FEATURED PRODUCTS - Mobile 3 | Tablet 3 | Desktop Auto */}
        <section className="w-full px-4 lg:px-20 py-24">
          <h2 className="text-2xl md:text-5xl font-black mb-12 italic uppercase border-l-8 border-amber-500 pl-4 md:pl-6 tracking-tight animate-reveal-left">
            FEATURED HARDWARE
          </h2>
          ...
          <div className="grid grid-cols-3 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-6 gap-2 md:gap-8">
            {products.length > 0 ? products.map((p, i) => (
              <div
                ...
                className="bg-zinc-900/40 border border-white/5 p-2 md:p-6 rounded-[15px] md:rounded-[35px] backdrop-blur-xl hover:border-amber-500/50 transition-all shadow-2xl overflow-hidden group cursor-pointer hover:scale-105 duration-300 animate-reveal-up fill-mode-both"
              >
                {/* Image Container - Mobile වලදී padding අඩු කළා */}
                ...
                {/* Product Name - Mobile වලදී ඉතා කුඩා අකුරු (text-[8px]) භාවිතා කළා */}
                <h3 className="font-bold text-[8px] md:text-xl mb-0.5 md:mb-2 truncate uppercase italic group-hover:text-amber-500 transition-colors">
                  {p.name}
                </h3>

                {/* Price - Mobile වලදී text-[10px] */}
                <p className="text-[10px] md:text-2xl font-black mb-2 md:mb-6 text-amber-500 italic">
                  {p.sellingPrice?.toLocaleString()}
                </p>

                {/* Button - Mobile වලදී කුඩා කර ඇත */}
                <button onClick={() => setCart([...cart, p])} className="w-full py-1.5 md:py-4 bg-white text-black rounded-md md:rounded-xl font-black hover:bg-amber-500 transition-all uppercase italic text-[7px] md:text-sm relative overflow-hidden">
                  ADD
                </button>
              </div>
            )) : (
              /* Skeleton Loaders */
              [1, 2, 3, 4, 5].map(n => <div key={n} className="h-[150px] md:h-[400px] bg-zinc-900/20 rounded-[15px] md:rounded-[35px] animate-pulse border border-white/5"></div>)
            )}
          </div>
        </section>
